...
This is the same product *shape* as roomGPT, but a fresh, more refined implementation. Reference roomGPT only for UX patterns (see §12), not its outdated internals.
...
1. **Upload** — drag-and-drop zone *and* click-to-browse. Show an immediate preview. Accept JPEG/PNG/WebP. **Compress/resize client-side** to ~1024–1536px longest edge before upload (faster, avoids body-size limits, lowers cost).
...
4. **Generate** — one clear primary action. Disabled until an image + style are chosen.
5. **Loading experience** — NOT a bare spinner. Use a tasteful animated state: a soft shimmer/grain over the source image + rotating descriptive captions (e.g., "Studying the room's light…", "Selecting materials…", "Styling the space…"). This perceived-progress moment is part of the wow.
6. **Before/after reveal (signature interaction)** — a draggable comparison slider over the original vs. restyled image. Must feel buttery: pointer + touch support, a clear handle, smooth motion. This is the centerpiece — invest here.
7. **Download** — download the restyled image at full resolution.
8. **Try another style / regenerate** — re-run on the same photo without re-uploading.
9. **Example gallery (demo safety net)** — 3–4 pre-baked before/after pairs the presenter can show instantly, so the demo is never empty and never depends solely on a live call landing well. Curate these to look excellent.
...
`lib/prompt.ts`:
```ts
export function buildPrompt(styleFragment: string, roomType: string) {
  return [
...
2. Scaffold Next.js + fonts + design tokens (globals.css, layout) — get the *look* right with static content.
